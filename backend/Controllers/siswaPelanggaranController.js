...
const tambahSiswaP = (req, res) => {
  const { siswa_id, pelanggaran_id, tanggal } = req.body;
  ...
  siswaPelanggaranModel.cekSiswa(siswa_id, (err, pelanggaranResult) => {
    ...
    if (pelanggaranResult.length === 0) return res.status(400).json({ error: "siswa_id tidak ditemukan" });

    const data = [siswa_id, pelanggaran_id, tanggal];
